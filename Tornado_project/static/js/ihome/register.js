...
function generateImageCode() {
    ...
    $(".image-code img").attr("src", "/api/imageccode?pcodeid="+picId+"&codeid="+ imageCodeId);
    imageCodeId = picId;
}
    ...
    $.ajax({
        ...
        success:function(data){
            if ("0" != data.errno){
                $("#image-code-err span").html(data.errmsg);
                $("#image-code-err").show();
                if (4002 == data.errno || 4003 == data.errno || 4004 == data.errno){
                    generateImageCode();
                }
                $(".phonecode-a").attr("onclick","sendSMSCode();");
            }
