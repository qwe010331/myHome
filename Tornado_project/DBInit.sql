-- CREATE DATABASE ihome DEFAULT CHARACTER utf-8;
...
CREATE TABLE ih_user_profile(
    ...
    up_real_name varchar(32) COMMENT '真实姓名',
    up_id_card varchar(20)  COMMENT '身份证号码',
...
CREATE TABLE ih_house_info (
    ...
    hi_max_days int unsigned NOT NULL DEFAULT '0' COMMENT '最长入住时间,0不限制',
    hi_order_count int unsigned NOT NULL DEFAULT '0' COMMENT '下单数量',
    hi_verify_status tinyint NOT NULL DEFAULT '0' COMMENT '
    审核状态,0-待审和,1-审核未通过,2-审核通过',
...
CREATE TABLE ih_order_info(
    ...
    oi_status tinyint NOT NULL DEFAULT '0' COMMENT '
        订单状态,0-待接单,1-待支付,2-已支付,3-待评价,4-已完成,5-已取消,6-拒接单',
    oi_comment text NULL COMMENT '订单评评论',
...
CREATE TABLE ih_house_image(
    ...
    hi_usr varchar(256) NOT NULL COMMENT '图片url',
    hi_citme datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
